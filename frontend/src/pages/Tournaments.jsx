...
// Link navigates to tournament detail page when clicked
import { Link } from 'react-router-dom';

import { getTournaments, createTournament } from '../api';

// Maps each status value to a display label and CSS class
const STATUS_BADGE = {
...
export default function Tournaments() {
  ...
  // Load tournaments when the page first opens
  useEffect(() => {
    fetchTournaments();
  }, []);
      ...
      <div className="card">
        ...
        {tournaments.length === 0 ? (
          ...
          <div className="tournament-list">
            {tournaments.map(t => {
              // Look up the badge info for this tournament's status
              ...
              return (
                <div key={t.id} className="tournament-item">
                  <div className="tournament-info">
                    ...
                    {/* Show current round number only while active */}
                    {t.status === 'active' && (
                  ...
                  {/* Button label changes based on tournament status */}
                  <Link
                    to={`/tournaments/${t.id}`}
